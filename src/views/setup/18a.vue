<!-- User will be automatically brought to this screen by a text file present on HW 5 which informs fox to redirect the user to this page (state variable setup2)-->
...
export default {
  ...
  components: {
    ...
    methods: {
        ...
        invoke('read_cd').then((res) => {
          ...
          for(let i = 0; i < resArray.length; i ++){
            ...
            if (String(it[0]).toUpperCase() == 'TYPE' && String(it[1]).toUpperCase() == 'SETUPCD'){
              ...
              this.loading = false
              this.$router.push({ name:'Setup18b' })
              break
            }
            else{
        ...
        .catch((e)=> {
          store.commit('setDebug', `error reading setup CD Setup18a-1: ${e}`)
          this.loading = false
          this.noCD = true
        })
        },
